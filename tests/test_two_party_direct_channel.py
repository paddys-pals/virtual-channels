...
def test_setStateWithoutStruct(w3, tester, channel_01, privkeys, finalize_period):
    ...
    # Test: succeeds
    channel_setStateWithoutStruct(w3, channel_01, state, sigs).call()

    # Test: `version` is not larger than the latest one, then fails.
    state_older_version = state.copy(version=0)
    with pytest.raises(TransactionFailed):
        channel_setStateWithoutStruct(w3, channel_01, state_older_version, sigs).call()
    ...
    with pytest.raises(TransactionFailed):
        channel_setStateWithoutStruct(w3, channel_01, state, sigs_copied).call()

    # Test: change states
    channel_setStateWithoutStruct(w3, channel_01, state, sigs).transact()
    ...
    channel_setStateWithoutStruct(w3, channel_01, state_1, sigs_1).call()
...
#     addr = channel_01.functions.recoverSignerWithoutStruct(
#         digest, v, r, s
#     ).call()
